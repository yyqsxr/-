...
function($) {
	...
	var Calendar = function(params) {
		...
		p.monthHTML = function(date, offset) {
			...
			for(i = 1; i <= rows; i++) {
				...
				for(j = 1; j <= cols; j++) {
					...
					rowHTML += '<div onClick="ajaxDate(this)" data-year="' + dayYear + '" data-month="' + dayMonth + '" data-day="' + dayNumber + '" class="picker-calendar-day' + (addClass) + '" data-date="' + (dayYear + '-' + dayMonth + '-' + dayNumber) + '"><span>' + dayNumber + '</span></div>';
				}
		...
		p.updateCurrentMonthYear = function(dir) {
			...
			p.container.find('.current-month-value').val(p.currentYear+'年'+p.params.monthNames[p.currentMonth]);
			p.container.find('.current-year-value').val(p.currentYear+'年');
	...
	defaults = $.fn.calendar.prototype.defaults = {
		...
		monthPickerTemplate: '<div class="picker-calendar-month-picker">' +
			'<a href="javascript:;" class="link icon-only picker-calendar-prev-month" onClick="months()"><i class="icon icon-prev"></i></a>' +
			'<input class="current-month-value" type="text" name="months" style="border:none;outline: inherit;" disabled="disbled">' +
			'<a href="javascript:;" class="link icon-only picker-calendar-next-month"  onClick="months()"><i class="icon icon-next"></i></a>' +
			'</div>',
		yearPicker: true,
